...
public class ParasiticManagerHooker {
    ...
    private static void sendBinderToManager(final ClassLoader classLoader, IBinder binder) {
        try {
            var clazz = XposedHelpers.findClass("org.mliboot.mlsp.manager.Constants", classLoader);
                ...
                for (var i = 0; i < param.args.length; ++i) {
                    if (param.args[i] instanceof ActivityInfo) {
                        ...
                        for (var activity : pkgInfo.activities) {
                            if ("org.mliboot.mlsp.manager.ui.activity.MainActivity".equals(activity.name)) {
                    ...
                    if (param.args[i] instanceof Intent) {
                        ...
                        intent.setComponent(new ComponentName(intent.getComponent().getPackageName(), "org.mliboot.mlsp.manager.ui.activity.MainActivity"));
                    }
                }
    ...
    private static void checkIntent(ILSPManagerService managerService, Intent intent) {
        ...
        if (intent.getCategories() == null || !intent.getCategories().contains("org.mliboot.mlsp.manager.LAUNCH_MANAGER")) {
