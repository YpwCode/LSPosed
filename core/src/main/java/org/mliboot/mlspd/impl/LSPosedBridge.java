...
public class LSPosedBridge {
    ...
    public static <T extends Executable> IXpInter.MethodUnhooker<T>
    doHook(T hookMethod, int priority, Class<? extends IXpInter.Hooker> hooker) {
        ...
        } else if (hooker.getAnnotation(XpHandler.class) == null) {
            throw new IllegalArgumentException("Hooker should be annotated with @XpHandler");
        }
        ...
        for (var method : hooker.getDeclaredMethods()) {
            if (method.getAnnotation(BeforeInvocation.class) != null) {
                ...
                if (params.length == 1) {
                    valid &= params[0].equals(IXpInter.BeforeHookCallback.class);
            ...
            if (method.getAnnotation(AfterInvocation.class) != null) {
                ...
                if (params.length == 1 || params.length == 2) {
                    valid &= params[0].equals(IXpInter.AfterHookCallback.class);
                } else if (params.length != 0) {
                    valid = false;
                }
                if (!valid) {
        ...
        if (HookBridge.hookMethod(true, hookMethod, LSPosedBridge.NativeHooker.class, priority, callback)) {
            return new IXpInter.MethodUnhooker<>() {
                @NonNull
                @Override
                public T getOrigin() {
                    return hookMethod;
        ...
        throw new HandlerFailedError("Cannot hook " + hookMethod);
    }
}
