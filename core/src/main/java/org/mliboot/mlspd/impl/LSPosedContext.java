...
import org.mliboot.mlspd.impl.utils.LSPosedDexParser;
...
import org.mliboot.libxp.api.XpModule;
...
public class LSPosedContext implements IXpInter {
    ...
    static final Set<XpModule> modules = ConcurrentHashMap.newKeySet();
    ...
    public static void callOnPackageLoaded(IXpModuleInter.PackageLoadedParam param) {
        for (XpModule module : modules) {
            try {
    ...
    public static void callOnSystemServerLoaded(IXpModuleInter.SystemServerLoadedParam param) {
        for (XpModule module : modules) {
            try {
    ...
    public static boolean loadModule(ActivityThread at, Module module) {
        try {
            ...
            var initLoader = XpModule.class.getClassLoader();
            var mcl = LspModuleClassLoader.loadApk(module.apkPath, module.file.preLoadedDexes, librarySearchPath, initLoader);
            if (mcl.loadClass(XpModule.class.getName()).getClassLoader() != initLoader) {
            ...
            for (var entry : module.file.moduleClassNames) {
                ...
                if (!XpModule.class.isAssignableFrom(moduleClass)) {
                    Log.e(TAG, "    This class doesn't implement any sub-interface of XpModule, skipping it");
                    continue;
                }
                try {
                    var moduleEntry = moduleClass.getConstructor(IXpInter.class, IXpModuleInter.ModuleLoadedParam.class);
                    var moduleContext = (XpModule) moduleEntry.newInstance(ctx, new IXpModuleInter.ModuleLoadedParam() {
                        @Override
