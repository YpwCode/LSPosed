...
import static org.mliboot.mlspd.service.LSPNotificationManager.SCOPE_CHANNEL_ID;
import static org.mliboot.mlspd.service.LSPNotificationManager.UPDATED_CHANNEL_ID;
import static org.mliboot.mlspd.service.PackageService.PER_USER_RANGE;
import static org.mliboot.mlspd.service.ServiceManager.TAG;
import static org.mliboot.mlspd.service.ServiceManager.getExecutorService;
...
import org.mliboot.libxp.service.IXpScopeCallback;
...
public class LSPosedService extends ILSPosedService.Stub {
    ...
    private void dispatchModuleScope(Intent intent) {
        ...
        var iCallback = IXpScopeCallback.Stub.asInterface(callback);
        try {
            var applicationInfo = PackageService.getApplicationInfo(scopePackageName, 0, userId);
            if (applicationInfo == null) {
                iCallback.onRequestFailed(scopePackageName, "Package not found");
                return;
            }
            ...
            switch (action) {
                case "approve" -> {
                    ...
                    iCallback.onRequestApproved(scopePackageName);
                }
                case "deny" -> iCallback.onRequestDenied(scopePackageName);
                case "delete" -> iCallback.onRequestTimeout(scopePackageName);
                case "block" -> {
                    ConfigManager.getInstance().blockScopeRequest(packageName);
                    iCallback.onRequestDenied(scopePackageName);
                }
            }
            Log.i(TAG, action + " scope " + scopePackageName + " for " + packageName + " in user " + userId);
        } catch (RemoteException e) {
            try {
                iCallback.onRequestFailed(scopePackageName, e.getMessage());
